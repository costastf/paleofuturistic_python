...
The template layers three things to test a generated project's code: pytest for the test runner, coverage for what got executed, and tox for the multi-version matrix. Each does one job.
...
If the template is being seeded into a codebase that's already covered, set `[tool.test-ratchet] mode = "strict"` in `pyproject.toml` to bypass the dormancy check entirely.
...
Under py310 the `else` runs; under py311+ the `if` does. No single env covers both, so each reports a miss on code the matrix as a whole exercises completely: on a scaffold with one such branch, each env reports 84.6% with one missing line and one missing branch, while the combined report is 100% with none. Averaging the percentages would give 84.6% for code that is fully exercised.

The union does not tell you that a line reached only under py314 also works under py310. Coverage measures reach, not correctness; the tests check the latter on every interpreter.
...
The template treats `_CI/` as part of the codebase for linting purposes but **not** for test coverage. The CI tooling isn't reasonably unit-testable — its job is to glue together external commands. Coverage of `_CI/` is implicitly via the workflow tasks running successfully end-to-end on every commit.

## See also

- [Choose the Python version range](../how-to/choose-python-version-range.md) — the practical commands for the version matrix.
- [Design principles](../../maintaining/explanation/design-principles.md) — why the template uses dependency groups instead of extras.
