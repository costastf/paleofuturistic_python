...
database at the moment it runs, not on your branch — so a pull request that touches a docstring
can go red because a CVE was published overnight, for something its author neither introduced
nor can fix. A pipeline that fails for reasons outside the author's control is one people learn
to ignore, so it is triggered on the axes it actually varies along:
...
- **On pushes that change the dependency surface** — `uv.lock`, `pyproject.toml`,
  `.security-overrides`. Change what you depend on and you own the result.
...
the wheel), but the audit is not: coupling them meant a newly published CVE made it impossible
to produce a wheel from code that built yesterday, blocking a hotfix on an advisory that had
nothing to do with it. `./workflow.cmd preflight --audit-dependencies` is how you run it
