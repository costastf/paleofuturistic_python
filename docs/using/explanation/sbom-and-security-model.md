...
`./workflow.cmd secure.audit` runs [pip-audit](https://github.com/pypa/pip-audit) over every package version in a generated project's `uv.lock`, plus `_CI/lib/vendor.txt`, comparing each against the [PyPI advisory database](https://github.com/pypa/advisory-database). `--strict`, so a package the service cannot resolve fails the audit rather than being listed in a skip table under exit 0.

Every version in the lockfile, not a rendering of it. Two narrower forms were tried and both saw less: `pip-audit` with no `-r` enumerates *installed* distributions, so a Linux runner omits whatever a platform marker, an extra or an unsynced group kept out of the venv; `uv export` keeps those markers in the file and pip-audit evaluates them, so the same pins are skipped one step later — on this scaffold an export audited on 3.10 skips `rpds-py==2026.6.3`, which four of the five matrix interpreters install and the SBOM lists. Reading `[[package]]` entries straight out of `uv.lock` has no markers to evaluate and no extras or groups to remember to ask for. One requirement file per version slot, because pip-audit refuses a file that names a package twice and a lockfile legitimately pins two versions behind different markers.

The vendored CI tree needs naming for a different reason: it ships as committed source with no `.dist-info`, so nothing that enumerates installed distributions can see it, and it is code `./workflow.cmd` executes in every job.

What it catches: a vulnerability with a known CVE/PYSEC ID affecting any locked version, or any vendored one.

What it doesn't catch:

- Zero-days (no advisory exists yet).
- A vendored file edited by hand — `vendor.txt` describes the packages that *should* be there.
- Anything in `[build-system] requires`, which nothing in this workflow audits.
