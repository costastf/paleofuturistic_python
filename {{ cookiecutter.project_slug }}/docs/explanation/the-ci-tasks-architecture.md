...
| `nox` | Python-native, env-isolating | Cookiecutter integration is messy; nox needs to BE installed before it can install anything |
...
`container.py` and `release.py` import these via a Jinja-substituted relative import — `from .{{ cookiecutter.git_hosting_service }} import …` — which renders at generation time to a concrete `from .github import …` or `from .gitlab import …`. After the post-gen hook deletes the unchosen module, there's exactly one code path and no runtime branching.
