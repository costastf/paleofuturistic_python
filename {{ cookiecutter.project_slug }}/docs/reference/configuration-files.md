...
The chosen host's CI config (only one of these exists per project, per the `git_hosting_service` answer). Edit to add jobs; preserve the existing lint/test/build flow if you want `cruft update` to keep working.

## `.cruft.json`

Cruft's state file. Records the template URL, the revision, and your answers. Don't edit unless you know cruft's update model — see the [cruft docs](https://cruft.github.io/cruft/#updating-a-project).
