...
Uses the detected container engine (docker or podman) to build `{{ cookiecutter.project_slug }}-deps:latest` from `Dockerfile.deps`. The build args are read from `[tool.docker-versions]` in `pyproject.toml`.
...
In CI, this delegates to the host-specific publish in `_CI/tasks/{{ cookiecutter.git_hosting_service }}.py`:
